...
echo
echo "服务器推荐:
润雨云(首选): 首月5折,半年7折,消费有返利 https://www.rainyun.com/hiram_
...
京东云: 2核2G3M云服务器仅58元 https://3.cn/20Z-Xyf4
百度云: 2核2G1M云服务器仅59元 https://cloud.baidu.com/campaign/ambassador-product/index.html?ambassadorId=2e198a5e0dac4c5aa9fe44457e2e9cb4#knowledge-bcc"
...
curl "http://gh.xisohi.dpdns.org/https://github.com/hjdhnx/hipy-sniffer/archive/refs/heads/main.zip" -sSLk -o ./hipyTmp/sniffer.zip -w "\nDownload complete. Total size: %{size_download} bytes. Speed: %{speed_download}\n"
...
if confirm "是否有私仓权限"; then
	...
	curl -H "Authorization: token $git_token" "http://gh.xisohi.dpdns.org/https://github.com/hjdhnx/hipy-server/archive/refs/heads/master.zip" -sSLk -o ./hipyTmp/server.zip -w "\nDownload complete. Total size: %{size_download} bytes. Speed: %{speed_download}\n"
	...
	echo "GIT_USER=" >> .env
	echo "GIT_TOKEN=" >> .env
	#curl "https://github.com/hjdhnx/hipy-server/archive/refs/heads/master.zip" -sSLk -o ./hipyTmp/server.zip
	curl "http://gh.xisohi.dpdns.org/https://github.com/sanshu-rom/server-release/releases/download/latest/hipy-server-latest.zip" -sSLk -o ./hipyTmp/server.zip -w "\nDownload complete. Total size: %{size_download} bytes. Speed: %{speed_download}\n"
	unzip -q ./hipyTmp/server.zip -d ./hipyTmp/server
